...
Важно: этот документ описывает текущее состояние кода в `src/` и `start_scripts/`. Исторические описания из `README.md`, где фигурирует `main.py`, не являются каноническими для текущей архитектуры.
